        ...
        for (i, child) in self.children.iter().enumerate() {
            ...
            match child {
                ...
                TexElement::Parent(p) => {
                    ...
                    output.extend(p.format(indent_level, tabstop, line_length));
                }
...
impl Formattable for TexCurlyGroup {
    fn format(&self, indent_level: usize, tabstop: usize, line_length: usize) -> Vec<String> {
        ...
        output.extend(bodytext.iter().map(|line| format!("{indent}{}",line.trim())));
        output.push("}".to_string());
        output
    }
}
...
//.children
//.iter()
//.map(|child| child.format(indent_level, tabstop, line_length)[0].clone())
//.collect::<Vec<_>>()
//.join(", ")]
//}
//}
